 ...
 * in full, half arena and foraging arena. (With skip_epoch_if_stuck)
 */
object E11 extends ExperimentSettings {
  ...
  def configVariation: Seq[Variation[Configuration, _]] = {
    ...
    val wholeArena = (("experiments/parametrized.argos", Map[String, String]("skip_epoch_if_stuck" -> "", "stuck_time" -> "8")), None)
    val halfArena = (("experiments/parametrized.argos", Map[String, String]("skip_epoch_if_stuck" -> "", "stuck_time" -> "8")),
      Some(HalfRegionVariation(region_nodes = 1, reset_region_every_epoch = true, penalty_factor = -1)))
    val foragingArena = (("experiments/parametrized-foraging.argos", Map("skip_epoch_if_stuck" -> "", "stuck_time" -> "8", "variant" -> "foraging", "light_nodes" -> "8", "light_threshold" -> "0.1")), None)
