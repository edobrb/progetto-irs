 ...
 */
object E13 extends ExperimentSettings {

  def defaultConfig: Configuration = Configuration(
    Simulation(
      argos = "",
      ticks_per_seconds = 10,
      experiment_length = 200 * 200, //200 edits
      robot_count = 10,
      print_analytics = true),
    Adaptation(epoch_length = 200,
      NetworkMutation(
  ...
  def configVariation: Seq[Variation[Configuration, _]] = {
    ...
    Seq(
      ...
      Variation(Seq(foragingArena2), arenaLens, "objective", (v: ((String, Map[String, String]), Option[HalfRegionVariation])) => v match {
