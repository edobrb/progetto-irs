 ...
 * in full, half arena and foraging arena. (with K=2).
 */
object E12 extends ExperimentSettings {
  ...
  def defaultConfig: Configuration = Configuration(
    ...
    Network(n = 100, k = 2, p = 0, self_loops = false, only_distinct_connections = true,
      io = NetworkIO(
  ...
  def configVariation: Seq[Variation[Configuration, _]] = {
    ...
    Seq(
      Variation(Seq(0.1, 0.5), lens(_.network.p), "p"),
