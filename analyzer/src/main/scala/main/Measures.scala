...
import play.api.libs.json.{JsArray, JsBoolean, JsError, JsNull, JsNumber, JsObject, JsResult, JsString, JsSuccess, JsValue, Json, Reads}
    ...
    val robotsData: MapView[Configuration, Seq[(Double, Double)]] = Loader.OUTPUT_FILENAMES(args).parMap(Args.PARALLELISM_DEGREE(args), { filename =>
      utils.File.read(filename).map({ str =>
        Try(readFromString[Seq[RobotData]](str)) match {
          ...
          case Success(robotsData) =>
            Right(robotsData.map {
      ...
      case Left(_) => Nil
    }.groupBy(_._1).view.mapValues(v => v.map(d => (d._2, d._3)).toSeq)
    ...
    robotsData.flatMap({
      ...
      case (config, data) =>
        ...
        series.zip(Seq(new Color(255, 0, 0), new Color(0, 255, 0), new Color(0, 0, 255))).foreach {
          case ((name, s), color) =>
            val chartSeries = chart.addSeries(name, s.map(_._2.toDouble).toArray, s.map(_._1).toArray)
            chartSeries.setMarkerColor(new Color(color.getRed, color.getGreen, color.getBlue, 20))
        }
