...
function BooleanNetwork:initialize(options)
  ...
  local function_length = 2^options["nodes_input_count"]
  self.boolean_functions = create_matrix(options["node_count"], function_length,
...
---@param input boolean
---@param index number
function BooleanNetwork:force_input_value(input, index)
  self.node_states[self.input_nodes[index]] = input
end
